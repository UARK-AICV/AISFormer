...
from .mask_head import build_mask_head, mask_rcnn_inference, mask_rcnn_loss, mask_rcnn_inference_amodal
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances], c_iter=None, dice_loss=False):
        ...
        if self.training:
            ...
            if self.aisformer.USE and self.aisformer.AMODAL_EVAL:
                if self.aisformer.AMODAL_EVAL == False:
                    mask_rcnn_inference(a_mask_logits, bo_masks, boundary, bo_bound, instances)
                else:
                    mask_rcnn_inference_amodal(mask_logits, a_mask_logits, bo_masks, invisible_mask_logits, instances)
            else:
