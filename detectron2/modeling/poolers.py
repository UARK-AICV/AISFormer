...
class ROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes], level=0, all_layers_roi_pooling=False):
        """
        Args:
        ...
        '''
        if all_layers_roi_pooling:	
            pooler = self.level_poolers[level]	
            level_assignments = torch.ones_like(level_assignments) * level	
            inds = torch.nonzero(level_assignments == level).squeeze(1)	
            pooler_fmt_boxes_level = pooler_fmt_boxes[inds]	
            assert len(output[inds]) > 0	
